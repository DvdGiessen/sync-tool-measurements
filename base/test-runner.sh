...
if [[ $VERBOSE -eq 1 ]] ; then
    exec 3> >(sed --unbuffered -r "s/.*/\x1B\[94m&\x1B\[0m/g" >&2)
else
    exec 3>/dev/null
fi
...
exec 2> >(tee "$LOGDIR/runner.log" >&2)

# Setup the sync tool
echo "[$(date +%s.%N)] Executing sync tool setup ..." >&2
if ! /synctool-setup.sh "runner" >&2 ; then
...
for TESTSCRIPT in /synctool-start.sh /tests/*.sh /synctool-stop.sh ; do
    ...
    # Check if the state is sane
    if [[ $PEERCOUNT -gt 0 ]] ; then
        STATE=$(find "$WORKDIR" \( ! -regex '.*/\..*' \) -type f -exec cksum {} \; | sort)
        if [[ $VERBOSE -eq 1 ]] ; then
            echo -e "[$(date +%s.%N)] Before state:\n$STATE" >&3
        fi
        if [[ "$STATE" != "$PREVIOUSSTATE" ]]; then
            echo -e "[$(date +%s.%N)] \e[91mThe state was altered between tests!\e[0m" >&2
        fi
    fi

    # Start inotifywait and tcpdump to monitor disk and network activity
    echo "[$(date +%s.%N)] Setting up monitoring ..." >&2
    inotifywait -mr "$WORKDIR" > >( while IFS= read -r line ; do echo "$(date +%s.%N) $line" ; done > "$LOGFILE-inotifywait.log" ) 2>&1 &
    INOTIFYWAIT_PID=$!
    tcpdump -i any -s0 -w "$LOGFILE-tcpdump.pcap" > "$LOGFILE-tcpdump.log" 2>&1 &
    TCPDUMP_PID=$!
    ...
    if [[ $PEERCOUNT -gt 0 ]] ; then
        ...
        STATE=$(find "$WORKDIR" \( ! -regex '.*/\..*' \) -type f -exec cksum {} \; | sort)
        if [[ $VERBOSE -eq 1 ]] ; then
            echo -e "[$(date +%s.%N)] After state:\n$STATE" >&3
        fi
        
        # Check for invalid state (non-)changes
        if [[ "$TESTSCRIPT" == "/synctool-start.sh" ]] || [[ "$TESTSCRIPT" == "/synctool-stop.sh" ]] ; then
            if [[ "$STATE" != "$PREVIOUSSTATE" ]] ; then
                echo -e "[$(date +%s.%N)] \e[91mThe state was altered after running $TESTSCRIPT!\e[0m" >&2
            fi
        else
            if [[ "$STATE" == "$PREVIOUSSTATE" ]] ; then
...
cd $LOGDIR
tar chf - $(find . -mindepth 1 -maxdepth 1 -exec basename {} \;)
exit 0
