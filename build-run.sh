...
# Usage / help text
print_usage() {
    echo "Usage: $0 SYNCTOOL PEERCOUNT"
    echo ""
...
# Build all images
echo "[$(date +%s.%N)] Building base Docker image ..." >&2
docker build -t "utw-cn:base" base > /dev/null
echo "[$(date +%s.%N)] Building $SYNCTOOL Docker image ..." >&2
docker build -t "utw-cn:$SYNCTOOL" "$SYNCTOOL" > /dev/null

# Create the test runner container
RUNNER=$(docker create -i --tmpfs "/workdir" "utw-cn:$SYNCTOOL" /test-runner.sh "$PEERCOUNT")
...
if [[ $PEERCOUNT -gt 0 ]] ; then
    ...
    for (( I=1 ; I <= PEERCOUNT; I++)) ; do
        PEERS[$((I - 1))]=$(docker create -ti --tmpfs /workdir --volumes-from "$RUNNER" "utw-cn:$SYNCTOOL" /test-peer.sh $I)
        echo -e "  \e[93m-  Press Ctrl-P Ctrl-Q to detach this peer after completing setup  -\e[0m  " >&2
...
if docker start -ai "$RUNNER" > results.tar ; then
     echo "[$(date +%s.%N)] Saved test results to \"results.tar\"."
else
